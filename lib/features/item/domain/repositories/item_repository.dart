...
class ItemRepository implements ItemRepositoryInterface {
  ...
  Future<BasicMedicineModel?> getBasicMedicine(DataSourceEnum source) async {
    ...
    String cacheId = '${AppConstants.basicMedicineUri}?offset=1&limit=50&moduleId=${Get.find<SplashController>().module!.id}';
  ...
  Future<List<Item>?> _getPopularItemList(String type, {required DataSourceEnum source}) async {
    ...
    String cacheId = '${AppConstants.popularItemUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id}';

    switch(source) {

      case DataSourceEnum.client:
        Response response = await apiClient.getData('${AppConstants.popularItemUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id}');
        if (response.statusCode == 200) {
          popularItemList = [];
  ...
  Future<ItemModel?> _getReviewedItemList(String type, {required DataSourceEnum source}) async {
    ItemModel? itemModel;
    String cacheId = '${AppConstants.reviewedItemUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id}';

    switch(source) {

      case DataSourceEnum.client:
        Response response = await apiClient.getData('${AppConstants.reviewedItemUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id}');
        if(response.statusCode == 200) {
  ...
  Future<ItemModel?> _getFeaturedCategoriesItemList({required DataSourceEnum source}) async {
    ...
    String cacheId = '${AppConstants.featuredCategoriesItemsUri}?limit=30&offset=1&moduleId=${Get.find<SplashController>().module!.id}';

    switch(source) {

      case DataSourceEnum.client:
        Response response = await apiClient.getData('${AppConstants.featuredCategoriesItemsUri}?limit=30&offset=1&moduleId=${Get.find<SplashController>().module!.id}');
        if (response.statusCode == 200) {
  ...
  Future<List<Item>?> _getRecommendedItemList(String type, {required DataSourceEnum source}) async {
    ...
    String cacheId = '${AppConstants.recommendedItemsUri}$type&limit=30&moduleId=${Get.find<SplashController>().module!.id}';

    switch(source) {

      case DataSourceEnum.client:
        Response response = await apiClient.getData('${AppConstants.recommendedItemsUri}$type&limit=30&moduleId=${Get.find<SplashController>().module!.id}');
        if (response.statusCode == 200) {
          recommendedItemList = [];
  ...
  Future<List<Item>?> _getDiscountedItemList(String type, {required DataSourceEnum source}) async {
    ...
    String cacheId = '${AppConstants.discountedItemsUri}?type=$type&offset=1&limit=50&moduleId=${Get.find<SplashController>().module!.id}';

    switch(source) {

      case DataSourceEnum.client:
        Response response = await apiClient.getData('${AppConstants.discountedItemsUri}?type=$type&offset=1&limit=50&moduleId=${Get.find<SplashController>().module!.id}');
        if (response.statusCode == 200) {
          discountedItemList = [];
