...
class FlashSaleRepository implements FlashSaleRepositoryInterface {
  ...
  Future<FlashSaleModel?> getFlashSale({required DataSourceEnum source}) async {
    ...
    String cacheId = '${AppConstants.flashSaleUri}&moduleId=${Get.find<SplashController>().module!.id!}';
  ...
  Future<ProductFlashSale?> getFlashSaleWithId(int id, int offset) async {
    ...
    Response response = await apiClient.getData('${AppConstants.flashSaleProductsUri}?flash_sale_id=$id&offset=$offset&limit=10&moduleId=${Get.find<SplashController>().module!.id}');
    if(response.statusCode == 200) {
