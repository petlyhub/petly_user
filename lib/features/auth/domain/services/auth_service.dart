...
/*  @override
  ...
  Future<void> _updateHeaderFunctionality(AuthResponseModel authResponse,
    ...
    if (authResponse.isEmailVerified! &&
      ...
      // 2️⃣ حفظ user_id 👇 (المهم هنا)
      // authRepositoryInterface.saveUserId(authResponse.user!.id.toString(),
      //     alreadyInApp: alreadyInApp);
...
//     // 🔴 الحل هنا: التحقق من أن التوكن موجود وليس null
...
//     } else {
//       // ⚠️ في حال رجع 200 بس التوكن نل (مثلاً الحساب محتاج تفعيل أو بيانات إضافية)
//       print("WARNING: Login succeeded but Token is NULL. User might need verification.");
      
//       // هنا رجع استجابة تفيد فلاتر أن العملية نجحت جزئياً لكن بدون توكن
//       return ResponseModel(false, "Verification required or incomplete profile", authResponseModel: authResponse);
//     }
...
// }
  ...
  // Future<void> saveTokenAutha() async {
  //   Response response =
  //       await authRepositoryInterface.loginWithSocialMedia();
