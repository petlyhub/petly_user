...
class StoreRepository implements StoreRepositoryInterface {
  ...
  Future<StoreModel?> _getStoreList(
    ...
    switch (source) {
      case DataSourceEnum.client:
        Response response =
            await apiClient.getData('${AppConstants.storeUri}/$filterBy'
  ...
  Future<List<Store>?> _getPopularStoreList(String type,
    ...
    String cacheId =
        '${AppConstants.popularStoreUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id!}';

    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient
            .getData('${AppConstants.popularStoreUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id!}');
        if (response.statusCode == 200) {
          popularStoreList = [];
    ...
    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
          '${AppConstants.latestStoreUri}?type=$type&moduleId=${Get.find<SplashController>().module!.id}',
        );
        print('Module ID: ${Get.find<SplashController>().module?.id}');
        if (response.statusCode == 200) {
          latestStoreList = [];
          response.body['stores']
  ...
  Future<List<Store>?> _getTopOfferStoreList(
    ...
    String cacheId =
        '${AppConstants.topOfferStoreUri}&moduleId=${Get.find<SplashController>().module!.id}';

    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
            '${AppConstants.topOfferStoreUri}?sort_by=$sortBy&${filterBy == '1' ? 'halal=1' : filterBy == 'veg' ? 'type=veg' : filterBy == 'non_veg' ? 'type=non_veg' : 'type='}&moduleId=${Get.find<SplashController>().module!.id}');
        if (response.statusCode == 200) {
          topOfferStoreList = [];
  ...
  Future<List<Store>?> _getFeaturedStoreList(
    ...
    String cacheId =
        '${AppConstants.storeUri}/all?featured=1&offset=1&limit=50&moduleId=${Get.find<SplashController>().module!.id}}';
  ...
  Future<ItemModel?> getStoreItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.storeItemUri}?store_id=$storeID&category_id=$categoryID&offset=$offset&limit=13&type=$type&moduleId=${Get.find<SplashController>().module!.id}');
    if (response.statusCode == 200) {
  ...
  Future<ItemModel?> getStoreSearchItemList(String searchText, String? storeID,
    ...
    Response response = await apiClient.getData(
        '${AppConstants.searchUri}items/search?store_id=$storeID&name=$searchText&offset=$offset&limit=10&type=$type&category_id=${categoryID ?? ''}&moduleId=${Get.find<SplashController>().module!.id}');
    if (response.statusCode == 200) {
  ...
  Future<RecommendedItemModel?> _getStoreRecommendedItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.storeRecommendedItemUri}?store_id=$storeId&offset=1&limit=50&moduleId=${Get.find<SplashController>().module!.id}');
    if (response.statusCode == 200) {
  ...
  Future<List<StoreBannerModel>?> _getStoreBannerList(int? storeId) async {
    ...
    Response response =
        await apiClient.getData('${AppConstants.storeBannersUri}$storeId&moduleId=${Get.find<SplashController>().module!.id}');
    if (response.statusCode == 200) {
      storeBanners = [];
      response.body.forEach(
